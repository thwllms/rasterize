...
def rasterize(polygon, field, cellsize, output_raster, value_to_nodata=-9999.0):
	...
	value = 1
	# Increment array value by 1 at each cell. This way when we convert to 
	# polygons, the shape of each individual cell is preserved. The
	# RasterToPolygon tool lumps adjacent cells together into single
	# polygons if they have the same value. We don't want that.
	...
	raster_int = arcpy.NumPyArrayToRaster(array_int, lowerleft, cellsize, cellsize)
	# Convert raster to polygon
	polygrid = 'in_memory/polygrid'
	arcpy.RasterToPolygon_conversion(raster_int, polygrid, 'NO_SIMPLIFY', 'VALUE')
	# Intersect
	...
	# Loop through the "intersected" layer and increment values in a numpy array.
	# Since arcpy doesn't recognize masked numpy arrays, we need to build our own
	# version of a mask. The "mask" array keeps track of which cells need to be
	# masked or unmasked. After the calculations take place, all "masked" cells
	# are set to NoData.
...
if __name__ == "__main__":
	...
	rasterize(polygonFeatureClass, fieldName, int(cellsize), outputRaster)
